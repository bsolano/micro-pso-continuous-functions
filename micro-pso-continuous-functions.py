...
def experiment_parallel_run_loop(parameters):
    ...
    pso = Solver(globals()[function], functions_search_space[function], max_epochs=500, population_size=10, beta=parameters[0], alfa=parameters[1], iterations=int(
        50 + (parameters[2] * (250 - 50))), crossover_type=crossover_type, mutation_type=mutation_type, mu=parameters[5], sigma=parameters[6], gamma=parameters[7])
    pso.run()  # runs the PSO algorithm
    cost = pso.getGBest().getCostPBest()
    return (cost, 1 if cost == 0.0 else 0, pso.getEpoch())
...
def experiment():
    ...
    parameters_space = [[9.23740174e-01, 4.18338939e-01, 1.16484035e-01, 4.68120881e-01,
                            8.64381462e-02, 4.28087985e-01, 5.55358928e-01, 4.31981332e-01],
