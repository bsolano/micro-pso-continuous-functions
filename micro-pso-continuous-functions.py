...
class Solver:
  ...
  def mutateGoodSolution(self, elite_solution, min, max):
    ...
    min = chromosome[point] - ((chromosome[point] - min) * (1-(self.last_epoch/self.max_epochs)))
    max = chromosome[point] + ((max - chromosome[point]) * (1-(self.last_epoch/self.max_epochs)))
